 ...
 * アクティビティの取得 (最新100件)
 */
export const getStravaActivities = async (accessToken: string, afterTimestamp: number) => {
 ...
 * 特定のアクティビティを取得
 */
export const getStravaActivityById = async (accessToken: string, activityId: string) => {
 ...
 */
export const syncSingleActivity = async (stravaAthleteId: string, stravaActivityId: string) => {
  // アスリートIDからユーザーを特定
  ...
  // トークンの有効期限チェック
  if (!user.stravaExpiresAt || user.stravaExpiresAt.getTime() < Date.now() + 5 * 60 * 1000) {
    accessToken = await refreshStravaToken(user.id);
  }

  try {
    const stravaAct = await getStravaActivityById(accessToken, stravaActivityId);
