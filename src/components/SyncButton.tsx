...
import { RefreshCw, CheckCircle2, Award, Leaf } from 'lucide-react';
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.newActivitiesCount > 0) {
          // 成果の通知
          toast.success(`${result.newActivitiesCount} 件のアクティビティを同期しました！`, {
          ...
          if (result.co2ReductionDelta > 0) {
            const lifespan = calculateEarthLifespanExtension(result.co2ReductionDelta);
          ...
          toast.info('新しいアクティビティはありませんでした。', {
            description: 'Stravaに最新の移動が記録されているか確認してください。'
          });
        }
        ...
        toast.error('同期に失敗しました', {
          description: 'Stravaとの接続状態を確認するか、しばらく時間をおいて試してください。'
        });
      }
    } catch (error) {
